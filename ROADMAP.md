...
- 🔄 **UX redesign (page-by-page)** — from a Figma direction, reskin every flow starting at the
  ...
  - **Travel/buffer time between appointments** — SP-controlled selector (None/15/30/45 min, default
    ...
    "Studio editing" item.
- 🔄 **Provider / treatment search** (28 Jun) — home hero now has a **separate provider-name search** + a **treatments multi-select dropdown** (chips, like service areas) + area; the "Browse treatments" circles sync to the same selection. Still client-side over listed providers; promote to server-side `/discover` (name, treatment, area) as provider count grows. NOTE: search only finds `is_listed=true` providers (the discovery gate) — listed Leroy (test acct) so he's findable; real providers list once reviewed.
...
- 🔄 **Studio editing** (28 Jun) — `/studio/edit` lets the provider edit **profile** (photo link, bio, service areas, who-they-serve, good-to-know, mobile toggle), **services** (add/edit price+duration/remove), and **availability** (days, hours, travel buffer). Owner-scoped RLS already allowed these writes (no new policies). Linked from studio header. STILL TODO: real **photo upload** (Supabase Storage) instead of a URL; headline stays auto (categories not editable here yet).
- ✅ **Provider info & pricing page** (28 Jun) — public `/for-providers` page explains the model (book-direct, keep 100%, own your clients, mobile-first), shows a **Free vs Pro** comparison (Free = available now; **Pro = "Coming soon · R99/mo"** so we don't promise unbuilt features), a 6-question FAQ, and repeats the "Become a Service Provider" CTA → `/join`. Linked from the home provider spotlight via a secondary **"See how it works →"** link and from the footer ("How it works"). Onboarding stays **start-free** — no plan-picker (per the no-paywall-in-onboarding decision).
- 🅿️ **Onboarding plan-picker + PayFast billing** — a Free/Pro selection at signup is **parked** until billing exists: PayFast recurring isn't wired up, so a plan choice would set a flag we can't charge against and only adds friction. Everyone onboards on Free; Pro becomes an upgrade flow (Plans screen) once PayFast is live. The Pro feature set advertised on `/for-providers` (deposits/payments, live WhatsApp+email reminders, featured placement, "on my way" + safety check-in, faster verification/branding, analytics) is mostly unbuilt — hence marketed as "coming soon."
